...
// AMDAL
	public function get_data_by_date_amdal($name = '', $product = '',  $min = '', $max = ''){
		...
		if (empty($product)) {
         ...
         $this->db->where('p.category_teknik','1');//dokumen lingkungan
	...
	// PERTEK
	public function get_data_by_date_ipal($name = '', $product = '',  $min = '', $max = ''){
		$this->db->select('
                          	tk.id,
							c.name,
							rp.id as recapitulation_id,
		...
		if (empty($product)) {
         ...
         $this->db->where('p.category_teknik','3');//ipal
        $this->db->order_by('rp.project_activity','DESC');
        $query = $this->db->get();
        // return $query->result();
	...
	function get_data_product_ipal(){
		return $this->db->query('SELECT * FROM amc_m_products WHERE category_teknik = 3');
